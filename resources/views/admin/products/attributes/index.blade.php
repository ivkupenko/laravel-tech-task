<x-app-layout>
    ...
    <div class="min-w-full py-8 px-4 flex flex-col items-center">
        <x-primary-link-button href="{{ route('admin.products.attributes.create') }}">Add Attribute</x-primary-link-button>
    </div>
            ...
            @forelse($attributes as $attribute)
                <tr class="border-b">
                    ...
                    <td class="px-4 py-2 text-center space-x-2">
                        <div>
                            <form method="POST" action="{{ route('admin.products.attributes.destroy', $attribute) }}" class="inline"
                                  onsubmit="return confirm('Delete this attribute?');">
                                @csrf
                                @method('DELETE')
                                <x-secondary-link-button href="{{ route('admin.products.attributes.edit', $attribute) }}">Edit
